...
export function assertFailure(
  ...
  chai.assert.deepEqual(
    fail.startPosition,
    ruleFail.getStartPosition().getLineAndCharacter(),
    `start char doesn't match for "${fail.message}"`
  );
  chai.assert.deepEqual(
    fail.endPosition,
    ruleFail.getEndPosition().getLineAndCharacter(),
    `end char doesn't match for "${fail.message}"`
  );
  if (result) {
...
export function assertFailures(ruleName: string, source: string, fails: IExpectedFailure[], options = null) {
  ...
  result.failures.forEach((ruleFail, index) => {
    ...
    chai.assert.deepEqual(
      fails[index].startPosition,
      ruleFail.getStartPosition().getLineAndCharacter(),
      `start char doesn't match`
    );
    chai.assert.deepEqual(
      fails[index].endPosition,
      ruleFail.getEndPosition().getLineAndCharacter(),
      `end char doesn't match`
    );
  });
  return result.failures;
}
